#!/bin/bash
# Run TensorTrust against every trained-model ablation in sequence.
#
...
#   bin/run_tensortrust_all_ablations.sh --format chat_template
#   bin/run_tensortrust_all_ablations.sh --splits hijacking
#   bin/run_tensortrust_all_ablations.sh --limit-hijacking 100 --limit-extraction 50
#   bin/run_tensortrust_all_ablations.sh --override tensortrust.generation_batch_size=2
#   bin/run_tensortrust_all_ablations.sh --include-stock
#   bin/run_tensortrust_all_ablations.sh --no-resume      # force fresh runs
#
# Parallel runs (e.g. across two GPUs or two terminals on a multi-GPU host):
#   # terminal 1
#   bin/run_tensortrust_all_ablations.sh --models gw_dpo,sft_only,standard_dpo
#   # terminal 2
#   bin/run_tensortrust_all_ablations.sh --models base_with_tokens,bilateral,three_level,tokens_only
#
...
#
# Per-model output lands at evaluation/external/tensortrust/<model>__<format>/run_<UTC-ts>/.
...
FORMAT="delimited"
SPLITS="hijacking,extraction"
LIMIT_HIJACKING_FLAG=()
LIMIT_EXTRACTION_FLAG=()
INCLUDE_STOCK=false
RESUME_FLAG="--resume"
EXTRA_OVERRIDES=()
MODELS_OVERRIDE=""
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --splits)
      SPLITS="$2"; shift 2 ;;
    --limit-hijacking)
      LIMIT_HIJACKING_FLAG=(--limit-hijacking "$2"); shift 2 ;;
    --limit-extraction)
      LIMIT_EXTRACTION_FLAG=(--limit-extraction "$2"); shift 2 ;;
    --override)
...
for model in "${MODELS[@]}"; do
  printf '\n==============================================================\n'
  printf '[%s] TensorTrust: %s (format=%s, splits=%s)\n' \
      "$(date '+%Y-%m-%d %H:%M:%S')" "$model" "$FORMAT" "$SPLITS"
  printf '==============================================================\n'
  if python bin/run_tensortrust.py \
        --model "$model" \
        --format "$FORMAT" \
        --splits "$SPLITS" \
        "$RESUME_FLAG" \
        ${LIMIT_HIJACKING_FLAG[@]+"${LIMIT_HIJACKING_FLAG[@]}"} \
        ${LIMIT_EXTRACTION_FLAG[@]+"${LIMIT_EXTRACTION_FLAG[@]}"} \
...
printf '\n==============================================================\n'
printf 'TensorTrust sweep complete in %ds across %d model(s)\n' "$elapsed" "${#MODELS[@]}"
if (( ${#failures[@]} > 0 )); then
